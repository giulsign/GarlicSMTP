...
def test_run_installation_uses_production_adapters_by_default(
    ...
    assert (
        received["python_module_available"]
        is probe_python_module_available
    )
...
def test_execute_installation_passes_python_module_detector_to_system():
    received = {}
...
def test_main_warns_that_new_login_is_required(
    tmp_path,
):
    messages = []
    ...
    main(
        project_root=tmp_path / "project",
        paths_factory=lambda: paths,
        installation=lambda **kwargs: None,
        password_reader=lambda prompt: "secret-password",
    ...
    assert messages == [
        (
            ...
            "Log out and log back in before starting GarlicSMTP "
            "so the Tor group membership becomes active."
        )
    ]
