...
def test_mail_flows_from_queue_to_transport(message):
...
def test_worker_delivers_correct_item(message):
...
def test_smtp_protocol_queues_message():
    ...
    assert item.message.headers.get("Subject") == "Flow Test"
    assert item.message.body == "Hello Flow"
